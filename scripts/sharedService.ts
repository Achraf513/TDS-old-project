import { feedBackPresenter } from './MVP/feedBack.js';
import { productItemModel, productItemPresenter } from './MVP/productItem.js';
...
class User {
    ...
    private ProductList: { [id: number]: productItemPresenter } = {};
    getProductList() { return this.ProductList; }
    appendProductsList(product: productItemPresenter) {
        this.ProductList[product.getModel().getId()] = product;
    }
    removeFromProductsList(product: productItemPresenter) {
...
export let feedBackList:Array<feedBackPresenter> = [];